...
1.  A basic grammar file [grammar](https://www.google.com/search?q=/nlp2025-2026/ass2/grammar). Make sure to review its format below.
2.  A python program [generate.py](https://www.google.com/search?q=/nlp2025-2026/ass2/generate.py) that generates sentences from a grammar.
...
While your new grammar may generate some very silly sentences, it should not generate any that are strictly **ungrammatical**. For example, in the following pair, your grammar must be able to generate the first sentence (11), but it must exclude the second one (12) marked with an asterix \*$^{\text{[1]}}$.

11. the president thought that a sandwich sighed .
12. \* the president thought that a sandwich sighed a pickle.
...
Think about all of the additional linguistic phenomena presented below, and extend your grammar from file `grammar2` to handle **TWO** of them – any two your choice. Briefly discuss your solutions and provide sample output.
...
Submit your two chosen and implemented extensions in a grammar file named `grammar4`, and 10 sentences generated by it in a file named `grammar4.gen`.

Use `test.py` to test your grammar if you find the file relevant.

-----

## Part B: Contextualized Word Vectors in Sequence Tagging (60 points)

This part will have you working with **contextualized word vectors** in the context of sequence tagging tasks.
...
The models in this assignment are small, and can also work on CPU. However, using **GPU** can really speed things up in terms of inference. While you are not required to use a GPU (and the assignment is designed to be possible without it), you can use Google Colab which gives access to sufficiently powered GPU.
...
The format is one sentence per line, the text is already pre-tokenized.
...
2.  do not tune... (The original text is cut off here, but context implies 'do not tune parameters' or 'do more than that').

#### B.1.1 Counts and Rules (15 points)

As a baseline, you will use only the counts of (word, tag) pairs from the training data. Given a word in a test sentence, you will predict the tag which has the highest co-occurrence count with that word in the training set (if a word is not in the training set, you will need to fall back to some other strategy, and you should not do more than that).
...
You are **not allowed** to train/fine-tune a classifier. You must work under the same constraints as in part B.1.
...
Describe your approach in the report, and submit your predictions over the test data. The predictions should be sent in a text file `NER_preds.txt`.
...
Your evaluation metrics in this part should be **precision**, **recall** and **F1** over predicted spans.
...
Your predictions on the test sets:
